...
async function seed() {
    try {
        ...
        const employees = [];
        const empPasswordHash = await bcryptjs_1.default.hash('employeepassword', 10);
        for (const emp of employeeData) {
            const newEmp = await models_1.Employee.create({
                ...emp,
                status: 'active',
            });
