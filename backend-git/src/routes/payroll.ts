...
import { Payroll, Employee, Attendance } from '../models';
...
router.get('/', isAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const records = await Payroll.findAll({
      where: whereClause,
      include: [{ model: Employee, as: 'employee', attributes: ['id', 'name', 'department', 'position', 'email'] }],
      order: [['month', 'DESC']],
    });
...
router.post('/calculate', isAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const employees = await Employee.findAll({ where: employeeQuery });
...
router.post('/:id/pay', isAdmin, async (req: Request, res: Response) => {
  try {
